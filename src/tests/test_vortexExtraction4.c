...
int main(int argc,char **argv){
  ...
  double parVortex[4*nVortex],x0[2],dx[2],xf[2],*sField=NULL,*gField;
  double x,y,v0y0 = 0.05,*vCatalog=NULL;
  ...
  parVortex[0]=1.; parVortex[1]=1.; parVortex[2]=-2.0; parVortex[3]=0.;
  ...
  {
    FILE *dadosout;
    dadosout=fopen("data/initVortexExtract-4.txt","w");
    ...
    dadosout=fopen("data/labelVortexExtract-4.txt","w");
    for(i=0;i<Height;i+=1){
      for(j=0;j<Width;j+=1){
        x = x0[0] + i*dx[0];
        y = x0[1] + j*dx[1];
